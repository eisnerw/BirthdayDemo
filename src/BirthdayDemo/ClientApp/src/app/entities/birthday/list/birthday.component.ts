...
export class BirthdayComponent implements OnInit {
  ...
  clearFilters(table: SuperTable, searchInput: any): void{
    ...
    Object.keys(this.expandedRows).forEach((key)=>{
      (this.expandedRows as any)[key] = false;
    });
  ...
  onRemoveChip(chip : any) : void {
    if ((this.expandedRows as any)[chip.id]){
      (this.expandedRows as any)[chip.id] = false;
    }
  ...
  doMenuView(selectedRow: any) : void {
    ...
    this.messageService.add({severity: 'success', summary: 'Row Viewed', detail: selected.lname as string });
  }
  ...
  handleNavigation(): void {
    combineLatest(this.activatedRoute.data, this.activatedRoute.queryParamMap, (data: Data, params: ParamMap) => {
      ...
      data['defaultSort'] = data['defaultSort'] || 'id,asc';
